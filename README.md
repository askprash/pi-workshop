...
- `/debate [--rounds 4] [--intensity normal|hard|ruthless] [--research] [--subagents] [--expert-subagents] [--fixed-experts] [--keep-dashboard] [--strong-model gpt-5.5] [--junior-model gpt-5.4-mini] <idea>`
- `/ideate [--rounds 4] [--intensity normal|hard|ruthless] [--research] [--subagents] [--expert-subagents] [--fixed-experts] [--keep-dashboard] [--strong-model gpt-5.5] [--junior-model gpt-5.4-mini] <idea>`
...
- explicit subagent workflow line showing whether parent-run briefs are enabled and whether main experts can call the `subagent` tool
- latest tool/activity snippets per expert when JSON events expose them, including `MAIN EXPERT called subagent tool` if direct expert delegation is enabled and observed
...
Direct main-expert subagent calls are disabled by default and are now made explicit in the dashboard plus `workflow.md`. To allow them, use:
...
- `workflow.md` — subagent/delegation policy for this run and each expert's tool list
